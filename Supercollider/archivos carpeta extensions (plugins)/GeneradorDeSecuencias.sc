...
Nodo {
	...
	*new { |nombres, arr, inicial = false|
	...
	/*
	* Método para comprobar si un acorde es una transición posible del nodo
	*/
	esAcordePosible{
		arg acorde;
	...
	/*
	* Método para comprobar si un acorde pertenece al nodo
	*/
...
/**
* Clase que almacena el grafo de transiciones e implementa las funciones
* necesarias para la generación de secuencias de acordes
*/
GeneradorSecuencias{
	...
	*new {
    ...
    /*
    * Función que inicializa los diccionarios de escalas y equivalencias
    */
    ...
    /*
    * Método para inicializar el array grafoTransiciones con todos los nodos del grafo
    */
	inicializarGrafoTransiciones{
		...
		~nodo5 = Nodo(['IIM', 'II7', 'II9', 'IIb9'], [], true);
		...
		~nodo21 = Nodo(['IM', 'I6', 'IM7', 'IM9'], [], true);
	...
	*/
	generarSecuencia{
        ...
        //Buscamos un acorde inicial de forma aleatoria
		while{~encontrado == false}{

            //Elegimos un nodo aleatoriamente
            ~nodoActual = grafoTransiciones.choose;

            //Si el nodo elegido es inicial actualizamos el escape
            if(~nodoActual.esInicial == true){
                ~encontrado = true;
            };
        };
		...
		for(1, numAcordes - 1){ //Puesto que ya hemos añadido el acorde inicial al principio
			~encontrado = false;
	...
	/**
	* Método para transformar la secuencia generada de grados genéricos a notas de una escala
    * concreta usando los diccionarios
	...
	*/
	transformarSecuecia{
        ...
        secuenciaGenerada.do{
			arg item, i; // item es el elemento, i su posición en el array secuenciaGenerada
    ...
    /**
	* Método para transformar la secuencia generada entre los modos mayor y menor
    * param: modo al que cambiar la secuencia
	*/
    cambiarModoSecuencia{
        ...
        secuenciaGenerada.do{
            ...
            if(modo == 'M'){
                secuenciaGenerada[i] = item.replace("m", "M");
            }{
                secuenciaGenerada[i] = item.replace("M", "m");
            };
	    };
